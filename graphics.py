...
def drawClosingScreen(windowSurface, race, results):
    ...
    tdAgents = list()
    for i in range(3):
        randomAgents.append(getAgentImage(race.randomRaceOrder[i], 200))
        adpAgents.append(getAgentImage(race.adpRaceOrder[i], 200))
        tdAgents.append(getAgentImage(race.tdRaceOrder[i], 200))
    sky = pygame.image.load("background.jpg")
    sky = pygame.transform.scale(sky, (1440, 850)) 
    font = pygame.font.SysFont("arial", 25, True, False)
...
def getAgentImage(agent, defSize = 60):
    image = None
